...
This guide covers creating, modifying, and deleting users and groups.
...
- This creates user 'john' with default settings. 

- What happens automatically: home directory at `/home/john`, default group named `john`, shell set to `/bin/bash`, and skeleton files copied from `/etc/skel`.
...
`cat /etc/shadow | grep john` — Shows password hash, last change date, and expiry information. Only root can read this file.
...
Always use `-r` if you want to completely remove the user's files. 

Without `-r` the home directory remains orphaned.
...
`sudo gpasswd -d john developers` — Removes user 'john' from group 'developers'.
